...
def process_pos_annotations_patient(src_path, patient_id):
    df_node = pandas.read_csv(settings.EXTRA_DATA_DIR+"luna16_annotations/annotations.csv")
...
def process_excluded_annotations_patient(src_path, patient_id):
    df_node = pandas.read_csv(settings.EXTRA_DATA_DIR+"luna16_annotations/annotations_excluded.csv")
    dst_dir = settings.LUNA16_EXTRACTED_IMAGE_DIR + "_labels/"
    if not os.path.exists(dst_dir):
        os.mkdir(dst_dir)
    dst_dir = dst_dir + patient_id + "/"
...
def process_luna_candidates_patient(src_path, patient_id):
    ...
    src_df = pandas.read_csv(settings.EXTRA_DATA_DIR+"luna16_annotations/" + "candidates_V2.csv")
