...
def process_pos_annotations_patient(src_path, patient_id):
    df_node = pandas.read_csv(settings.EXTRA_DATA_DIR+"resources/luna16_annotations/annotations.csv")
...
def process_excluded_annotations_patient(src_path, patient_id):
    df_node = pandas.read_csv(settings.EXTRA_DATA_DIR+"resources/luna16_annotations/annotations_excluded.csv")
    dst_dir = settings.LUNA16_EXTRACTED_IMAGE_DIR + "_labels/"
    if not os.path.exists(dst_dir):
        os.mkdir(dst_dir)
    dst_dir = dst_dir + patient_id + "/"
...
def process_luna_candidates_patient(src_path, patient_id):
    ...
    src_df = pandas.read_csv(settings.EXTRA_DATA_DIR+"resources/luna16_annotations/" + "candidates_V2.csv")
...
def process_lidc_annotations(only_patient=None, agreement_threshold=0):
    ...
    neg_count = 0
    all_lines = []
    for anno_dir in [d for d in glob.glob(settings.EXTRA_DATA_DIR+"resources/luna16_annotations/*") if os.path.isdir(d)]:
